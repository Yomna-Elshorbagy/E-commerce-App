...
export const addToCart = catchAsyncError(async (req, res, next) => {
  //get data from req
  let { productId, quentity } = req.body;
  ...
  //check stock
  if (!productExisit.instock(quentity)) {
  ...
  const userCart = await Cart.findOneAndUpdate(
    ...
    },
    {
      ...
      $inc: { "products.$.quentity": quentity }, //$ this for updting this product quentity
    },
    {
      new: true,
    }
  ...
  if (!userCart) {
    await Cart.findOneAndUpdate(
      ...
      {
        $push: {
          products: { productId, quentity, price: productExisit.price },
        },
      },
...
export const updateQuentity = catchAsyncError(async (req, res, next) => {
  let { id } = req.params;
